...
void dispatch_qp_0(mb_transport *mb_trns, uint8_t *data) {
...
//	printf("from qp_num: %d\n", ntohs(mb_trns->source_qp));
	...
	if (qp && (qp->remote_qp_num || received_state < 3)) {	
		if (qp->state >= QPS_RTR && qp->remote_qp_num != ntohs(mb_trns->source_qp)) {
			printf("dropped packet, %d,%d,%d\n", qp->state, qp->remote_qp_num,ntohs(mb_trns->source_qp));
			return;
		}
	//	if (received_state > 1 && qp->remote_qp_num != ntohs(mb_trns->source_qp)) {
	//		return;
	//	}

		if (qp->qp_num != 0 && received_state > 2 && qp->remote_qp_num != ntohs(mb_trns->source_qp)) {
			printf("discarding for qp:%d for state:%d remote:%d,%d\n",qp->qp_num, received_state, qp->remote_qp_num, ntohs(mb_trns->source_qp));
		//	exit(1);
			return;
		}
...
int main(int argc, char *argv[]) {
	...
	setsockopt(raw_socket, SOL_SOCKET, SO_BINDTODEVICE, eth_interface, strlen(eth_interface));
	uint8_t dest_mac[6];
...
//	printf("_POSTED_RECV WR\n");

	int post_num = 10;
	for (int i = 0; i < post_num; i++) {
		post_send(qp, &minimal_wqe_send);
	}
//	printf("_POSTED_SEND, WAITING 3 SEC\n");
